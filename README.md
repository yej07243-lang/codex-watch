...
| `PROCESS_DOWN_CONFIRM` | 3 | 进程停止确认次数（连续 N 次无进程才通知飞书） |
...
**确认机制**: 进程消失后不会立即通知，而是等待连续 3 次检测（默认 45 秒）都无进程后才发送飞书通知。如果在确认窗口内 Codex 恢复运行（如临时重启），则不会发送通知。这避免了 Codex 升级、崩溃自恢复等短暂中断引发的假警报。
